...
library(rvest)
library(tidyverse)
page <- html_session('https://www.eur.nl/en/research/phd-programmes')
```
...
See https://www.dropbox.com/s/mqlmcqozb0bm8q0/job_browsing_handout.html?dl=1 or https://goo.gl/dD4Dhv
...
smoking %>% rename(country = `Country`, num_per_adult = `Cigarette consumption[8]`) %>% tbl_df
```
</div>
...
doc %>% html_nodes('table.wikitable') %>% map(~html_table(.)) %>% bind_rows
```

* Use `html_table()` to extract the first table (English language, daily) at https://en.wikipedia.org/wiki/List_of_newspapers_in_the_Republic_of_Ireland 
...
load_cached_oath_credentials <- safely( function() {
  token_env <- new.env()
  load(here::here("oauth_cache.RData"), token_env) 
...
if(!is.null(load_result$error)){
  stop('OAuth credentials need to be cached before this code can run. See "session_3.Rmd" at or near line 553 for instructions.')
